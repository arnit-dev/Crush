...
This package provides the stable, public Go API for [Crush](https://github.com/charmbracelet/crush) — a terminal-based AI coding assistant by [Charm](https://charm.sh).

It exposes the core types and interfaces needed to programmatically configure, initialize, and interact with Crush from external Go code. Types re-exported here are guaranteed to remain stable across minor releases.
...
`pkg/crush` is **experimental**. Types and functions exported from this package are not yet covered by semantic-versioning guarantees and may change in any release. Once the API has matured and stabilised, it will be promoted to a formal stability guarantee.

Internal packages under `internal/` are not covered by any guarantee and may change at any time.
